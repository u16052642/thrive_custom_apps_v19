...
class OnlyofficeDocuments(models.Model):
    _name = "onlyoffice.thrive.documents"
    ...
    def advanced_share_save(self, vals):
        ...
        access = self.env["onlyoffice.thrive.documents.access"].search([("document_id", "=", document_id)], limit=1)
        if not access:
            access = self.env["onlyoffice.thrive.documents.access"].create(
                {
        ...
        if users_to_remove:
            self.env["onlyoffice.thrive.documents.access.user"].search(
                [("document_id", "=", document_id), ("user_id", "in", users_to_remove)]
            ).unlink()

        user_accesses = vals.get("user_accesses", [])
        existing_accesses = (
            self.env["onlyoffice.thrive.documents.access.user"]
        ...
        for user_data in user_accesses:
            if user_data["user_id"] in existing_accesses:
                ...
                if self.env["res.partner"].search_count([("id", "=", user_data["user_id"])]):
                    self.env["onlyoffice.thrive.documents.access.user"].create(
                        {
                            "document_id": document_id,
