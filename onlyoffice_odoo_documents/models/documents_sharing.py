...
class DocumentsSharing(models.TransientModel):
    ...
    def _get_update_rights_params(self):
        ...
        original_user_roles = {}
        for access_user in self.env["onlyoffice.thrive.documents.access.user"].search([("document_id", "=", doc_id)]):
    ...
    def action_open(self, document_ids):  # noqa: C901
        ...
        for doc in documents:
            onlyoffice_user_accesses = self.env["onlyoffice.thrive.documents.access.user"].search(
                [("document_id", "=", doc.id)]
            )
        ...
        for field in ("access_internal", "access_via_link"):
            ...
            onlyoffice_values = []
            for doc in documents:
                onlyoffice_access = self.env["onlyoffice.thrive.documents.access"].search(
