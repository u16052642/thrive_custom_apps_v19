...
from thrive.addons.onlyoffice_odoo.controllers.controllers import Onlyoffice_Connector
from thrive.addons.onlyoffice_odoo.utils import config_utils, file_utils, jwt_utils, url_utils
...
class OnlyofficeDocuments_Connector(http.Controller):
    ...
    def post_file_create(self, folder_id, supported_format, title, url=None):
        ...
        try:
            ...
            request.env["onlyoffice.thrive.documents.access"].create(
                {
            ...
            request.env["onlyoffice.thrive.documents.access.user"].create(
                {
                    "document_id": document.id,
...
class OnlyofficeDocuments_Inherited_Connector(Onlyoffice_Connector):
    ...
    def prepare_share_editor(self, document, access_token):
        role = None
        access = (
            request.env["onlyoffice.thrive.documents.access"].sudo().search([("document_id", "=", document.id)], limit=1)
        )
        if access:
    ...
    def share_callback(self, access_token, oo_security_token):
        ...
        try:
            ...
            access = (
                request.env["onlyoffice.thrive.documents.access"]
                .sudo()
